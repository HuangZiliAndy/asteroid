...
storage_dir=/home/zilhuang/data
...
if [[ $stage -le  1 ]]; then
  ...
  python local/create_local_metadata.py --librimix_dir $storage_dir/Libri2Mix
fi
...
expdir=exp/train_convtasnet_tera
...
if [[ $stage -le 2 ]]; then
  ...
  mkdir -p logs
  CUDA_VISIBLE_DEVICES=$id $python_path train_pretrain.py --exp_dir $expdir \
		--n_blocks $n_blocks \
		--n_repeats $n_repeats \
		--mask_act $mask_act \
		--epochs $epochs \
